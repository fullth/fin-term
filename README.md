...
| `:crypto` | `:hold` | 코인 보유·차트 패널 토글 (업비트 실시간 KRW) |
| `:coin BTC` | | 차트·상세 대상 코인 선택 |
...
`:crypto` 로 **코인 보유·실시간 시세·캔들 차트** 패널을 켭니다 (기본 숨김 — 화면을 늘리지 않아 평소 깜빡임 없음). 업비트 웹소켓으로 BTC·ETH·XRP·BCH 의 원화 시세를 실시간 수신하고, 보유 내역과 묶어 평가금액·손익·수익률을 보여줍니다. 키 불필요.

보유 내역은 `~/.fin-term/holdings.json` 에 둡니다 (없으면 코인 패널은 시세만, 손익 없이 동작):
...
`:coin BTC` 로 차트 대상 코인을, `:chart` 로 기간(1분~1달)을 바꿉니다.

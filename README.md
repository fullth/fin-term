...
`:brief` 는 최근 뉴스를 바탕으로 Claude 가 국내외 주식시장 정세 브리핑(요약 + 해외/국내 시장 + 주요 테마 + 오늘의 키워드 + 흐름 메모)을 한국어로 생성합니다. 투자 초보자가 흐름을 파악하기 쉽도록 풀어서 설명합니다. `ANTHROPIC_API_KEY` 환경변수가 있어야 동작하며, 호출 비용이 발생합니다. `Esc` 로 닫습니다.
